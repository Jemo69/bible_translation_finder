...
def test_bible_to_opensong_xml(bible, tmp_path):
    xml_str = bible.to_opensong_xml()
    assert xml_str.startswith('<?xml version="1.0" encoding="UTF-8"?>')
    reloaded = Bible.from_opensong_xml(xml_str, translation="TEST")
    assert reloaded.get_verse("John", 3, 16).text == bible.get_verse("John", 3, 16).text
    out_file = tmp_path / "saved.xml"
    bible.save(out_file)
    assert out_file.exists()
    from_file = Bible.from_file(out_file, translation="TEST")
    assert from_file.get_verse("Genesis", 1, 1).text == bible.get_verse("Genesis", 1, 1).text


def test_top_level_helpers(tmp_path):
    import btm
    v = btm.get_verse("Genesis 1:1", data_dir=tmp_path)
    assert "beginning" in v.text.lower()
    p = btm.get_passage("Genesis 1:1-2", data_dir=tmp_path)
    assert len(p) == 2
    hits = btm.find("God", limit=5, data_dir=tmp_path)
    assert len(hits) > 0


def test_btf_import_alias():
    import btf
    assert hasattr(btf, "get_verse")
