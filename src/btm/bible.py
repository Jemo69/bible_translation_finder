...
def _parse_opensong_element(root: ET.Element, translation: str = "") -> _Books:
    ...
    for b in root.findall("b"):
        ...
        try:
            ...
            if raw_name.isdigit() and 1 <= int(raw_name) <= len(BIBLE_BOOKS):
                name = BIBLE_BOOKS[int(raw_name) - 1]
            else:
...
class Bible:
    ...
    # -- export -------------------------------------------------------
    def to_opensong_xml(self) -> str:
        """Export this Bible back to an OpenSong XML string."""
        ...
        for bname in BIBLE_BOOKS:
            if bname in self._books:
                ...
                for cnum in sorted(chapters.keys()):
                    ...
                    for vid in sorted(verses.keys(), key=_verse_sort_key):
                        v = ET.SubElement(c, "v", {"n": str(vid)})
                        v.text = verses[vid]
        return '<?xml version="1.0" encoding="UTF-8"?>\n' + ET.tostring(root, encoding="unicode")

    to_xml = to_opensong_xml

    def save(self, path: Union[Path, str]) -> Path:
        """Save this Bible to an OpenSong XML file."""
        dest = Path(path).expanduser()
        dest.parent.mkdir(parents=True, exist_ok=True)
        dest.write_text(self.to_opensong_xml(), encoding="utf-8")
        return dest
